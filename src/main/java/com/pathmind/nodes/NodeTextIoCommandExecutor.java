    ...
    void executeWriteBookCommand(CompletableFuture<Void> future) {
        ...
        client.execute(() -> {
            try {
                ...
                boolean useFilterables = false;
                if (!pages.isEmpty()) {
                    useFilterables = !(pages.get(0) instanceof String);
                ...
                while (pageCount <= pageIndex) {
                    if (appendNewPageMethod != null) {
                        ...
                        if (pages.size() == beforeSize && pageCount == beforeSize) {
                            pages.add(useFilterables ? Filterable.passThrough("") : "");
                            pageCount = pages.size();
                        }
                    } else {
                        pages.add(useFilterables ? Filterable.passThrough("") : "");
                ...
                if (!setViaMethod && pageIndex >= 0 && pageIndex < pages.size()) {
                    pages.set(pageIndex, useFilterables ? Filterable.passThrough(truncatedText) : truncatedText);
                    if (pagesField != null) {
