...
public class VillagerTradeSelector {
    ...
    public int render(GuiGraphics context, Font textRenderer, int x, int y, int width, int mouseX, int mouseY, float alpha) {
        ...
        if (showPlaceholder) {
            context.drawString(
                textRenderer,
                Component.literal("Search trades..."),
    ...
    private void loadProfessionOptions() {
        ...
        for (ResourceLocation id : BuiltInRegistries.VILLAGER_PROFESSION.keySet()) {
            ...
            Holder<VillagerProfession> entry = BuiltInRegistries.VILLAGER_PROFESSION.getHolder(key).orElse(null);
            if (entry == null) {
                continue;
            }
            ResourceKey<VillagerProfession> resolvedKey = entry.getKey();
    ...
    private void rebuildTrades() {
        ...
        Map<VillagerProfession, it.unimi.dsi.fastutil.ints.Int2ObjectMap<VillagerTrades.ItemListing[]>> primary =
            VillagerTrades.TRADES;
        Map<VillagerProfession, it.unimi.dsi.fastutil.ints.Int2ObjectMap<VillagerTrades.ItemListing[]>> secondary =
        ...
        net.minecraft.server.level.ServerLevel serverLevel = client.getSingleplayerServer() != null
            ? client.getSingleplayerServer().getLevel(net.minecraft.world.level.Level.OVERWORLD)
            : null;
        Level fallbackLevel = client.level;
        Level activeLevel = serverLevel != null ? serverLevel : fallbackLevel;
        if (activeLevel == null) {
            return;
        }
        ...
        for (it.unimi.dsi.fastutil.ints.Int2ObjectMap.Entry<VillagerTrades.ItemListing[]> entry : levelMap.int2ObjectEntrySet()) {
            ...
            for (VillagerTrades.ItemListing factory : factories) {
                ...
                MerchantOffer offer = createOffer(factory, serverLevel, fallbackLevel, villager, random);
                if (offer == null) {
                    continue;
                }
            ...
            if (!levelHasOffers && serverLevel != null) {
                MerchantOffers offers = villager.getOffers();
                if (offers != null) {
                    offers.clear();
                }
                invokeFillRecipes(villager, serverLevel);
    ...
    private it.unimi.dsi.fastutil.ints.Int2ObjectMap<VillagerTrades.ItemListing[]> resolveTradeLevels(
        ...
        if ((levelMap == null || levelMap.isEmpty()) && option.entry != null) {
            ResourceKey<VillagerProfession> resolvedKey = option.entry.getKey();
            if (resolvedKey != null) {
        ...
        if (levelMap == null || levelMap.isEmpty()) {
            for (Map.Entry<?, it.unimi.dsi.fastutil.ints.Int2ObjectMap<VillagerTrades.ItemListing[]>> entry : map.entrySet()) {
                ...
                if (key instanceof ResourceKey<?> registryKey) {
                    ...
                    ResourceKey<?> registryKey = registryEntry.getKey();
                    if (registryKey != null) {
                        ResourceLocation id = registryKey.location();
                        keyPath = id != null ? id.getPath() : null;
                    } else {
    ...
    private void updateSearchCaretFromClick(double mouseX) {
        ...
        int totalWidth = lastFont.width(value);
        ...
        for (int i = 1; i <= value.length(); i++) {
            int width = lastFont.width(value.substring(0, i));
    ...
    private static MerchantOffer createOffer(VillagerTrades.ItemListing factory, net.minecraft.server.level.ServerLevel serverLevel,
                                   Level fallbackLevel, Villager villager, RandomSource random) {
        ...
        try {
            for (Method method : factory.getClass().getMethods()) {
                ...
                if (params.length == 3) {
                    if (serverLevel != null && params[0].isInstance(serverLevel) && params[1].isInstance(villager)) {
                        Object offer = method.invoke(factory, serverLevel, villager, random);
                        if (offer instanceof MerchantOffer tradeOffer) {
                            return tradeOffer;
                        }
                    }
                    if (fallbackLevel != null && params[0].isInstance(fallbackLevel) && params[1].isInstance(villager)) {
                        Object offer = method.invoke(factory, fallbackLevel, villager, random);
                        if (offer instanceof MerchantOffer tradeOffer) {
                            return tradeOffer;
                        }
                    }
    ...
    private static void invokeFillRecipes(Villager villager, net.minecraft.server.level.ServerLevel serverLevel) {
        if (villager == null || serverLevel == null) {
            return;
        }
        try {
            Method method = Villager.class.getDeclaredMethod("fillRecipes", net.minecraft.server.level.ServerLevel.class);
            method.setAccessible(true);
            method.invoke(villager, serverLevel);
        } catch (ReflectiveOperationException ignored) {
    ...
    private int applyAlpha(int color, float alpha) {
        ...
        int applied = Math.round(baseAlpha * Mth.clamp(alpha, 0f, 1f));
